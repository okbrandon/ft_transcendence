...
class TournamentManager:
    ...
    async def start_next_match(self):
        ...
        return {'matchID': match.matchID, 'players': players}
...
class TonneruConsumer(AsyncJsonWebsocketConsumer):
    ...
    async def tournament_match_join(self, event):
        ...
        logger.info(f"Tournament match {match_id} is ready to begin for user {self.user.userID}")
    ...
    async def start_next_match(self):
        ...
        if current_match:
            if 'winner' in current_match:
                await self.channel_layer.group_send(
                    f"tournament_{self.tournament.tournamentID}",
                    {"type": "tournament.end", "tournamentID": current_match['tournamentID'], "winner": current_match['winner']}
                )
            else:
                await self.channel_layer.group_send(
                    f"tournament_{self.tournament.tournamentID}",
                    {"type": "match.begin", "matchID": current_match['matchID'], "players": current_match['players']}
                )
