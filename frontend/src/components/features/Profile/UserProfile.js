...
const UserProfile = () => {
	...
	useEffect(() => {
		GetUser().then((response) => { setUser(response); console.log(response); })
	}, []);
	...
	return (
		<UserProfileContainer>
			...
			<UserInfoContainer>
				<h2 style={{fontSize: '2rem'}}><i className="bi bi-circle-fill" style={{ fontSize: '20px', color: 'green' }}></i> {user.displayName}</h2>
